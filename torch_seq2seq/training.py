...
def main(datasetname):
    ...
    with open(f'../models/{datasetname}_log.txt', 'w') as logfile:
        for lr_val, hid_val, drop_val, emb_val in product(lr_range, hid_range, drop_range, emb_range):
            ...
            lr = lr_val
            encoder = EncoderGRU(src_vocab_size, hidden_size, emb_hidden_size, dropout_rate)
            decoder = DecoderGRU(tgt_vocab_size, hidden_size, emb_hidden_size, dropout_rate, device=device)
            seq2seq = Seq2SeqGRU(encoder, decoder)
