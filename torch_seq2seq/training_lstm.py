...
from model import EncoderGRU, DecoderGRU, Seq2SeqGRU, EncoderLSTM, DecoderLSTM, Seq2SeqLSTM
...
def main(datasetname):
    ...
    with open(f'../models/{datasetname}_log.txt', 'w') as logfile:
        for lr_val, hid_val, drop_val, emb_val, layer_val in product(lr_range, hid_range, drop_range, emb_range, layers_range):
            print('Params:', lr_val, hid_val, drop_val, emb_val, layer_val)
            ...
            lr = lr_val
            n_layers = layer_val
            encoder = EncoderLSTM(src_vocab_size, hidden_size, emb_hidden_size, dropout_rate, n_layers)
            decoder = DecoderLSTM(tgt_vocab_size, hidden_size, emb_hidden_size, dropout_rate, device=device, n_layers=n_layers)
            seq2seq = Seq2SeqLSTM(encoder, decoder)
